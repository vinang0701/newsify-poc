...
const InstitutionsMgmtPage = () => {
	...
	const fetchInstitutions = async () => {
		...
		try {
			...
			setError(
				e.response?.data?.detail ?? "Failed to fetch institutions",
			);
		} finally {
			setLoading(false);
		}
	...
	const handleCreate = async () => {
		...
		try {
			...
			setError(
				e.response?.data?.detail ?? "Failed to create institution",
			);
		}
	};
	// ── Update ──
	...
	const handleUpdate = async () => {
		...
		try {
			...
			setError(
				e.response?.data?.detail ?? "Failed to update institution",
			);
		}
	};
	...
	const handleSuspend = async () => {
		try {
			...
			setError(
				e.response?.data?.detail ?? "Failed to suspend institution",
			);
		}
	};
	...
	const handleActivate = async (id: string) => {
		...
		try {
			...
			setError(
				e.response?.data?.detail ?? "Failed to activate institution",
			);
		}
	};
				...
				<div style={s.tableHeader}>
					...
					<div style={{ position: "relative" }}>
						<button
							style={{
								...s.sortBtn,
								background: sortField ? "#E8ECF2" : "#F4F6FA",
								fontWeight: sortField ? 600 : 400,
							}}
							onMouseEnter={(e) =>
								(e.currentTarget.style.background = "#D1D5DB")
							}
							onMouseLeave={(e) =>
								(e.currentTarget.style.background = sortField
									? "#E8ECF2"
									: "#F4F6FA")
							}
						...
						>
							{sortField
								? `${sortField.replace("_", " ").charAt(0).toUpperCase() + sortField.replace("_", " ").slice(1)} ▾`
								: "Sort ▾"}
						</button>
						{showSortMenu && (
							<div
				...
				>
					...
					>
						...
						>
							...
							<strong>{selectedInst?.name}</strong>?
						</p>
						<p style={{ fontSize: 13, color: "#6B7280" }}>
							Access will be disabled but all data will be
							retained.
						</p>
						<div style={s.modalActions}>
							<button
...
const ModalFormFields = ({
	...
	<>
		<div style={s.formGroup}>
			<label style={s.label}>
				Institution Name <span style={s.required}>*</span>
			</label>
			<input
				style={{
					...s.input,
					borderColor: errors.name ? "#EF4444" : "#E8ECF2",
				}}
				value={form.name}
				onChange={(e) =>
					setForm((f) => ({
		...
		<div style={s.formGroup}>
			<label style={s.label}>
				Institution Email <span style={s.required}>*</span>
			</label>
			<input
				style={{
					...s.input,
					borderColor: errors.email ? "#EF4444" : "#E8ECF2",
				}}
				type="email"
				value={form.email}
				onChange={(e) =>
		...
		<div style={s.formGroup}>
			<label style={s.label}>
				Subscription Plan <span style={s.required}>*</span>
			</label>
			<select
				style={{
					...s.select,
					borderColor: errors.plan ? "#EF4444" : "#E8ECF2",
				}}
				value={form.plan}
				onChange={(e) =>
		...
		<div style={s.formGroup}>
			<label style={s.label}>
				Start Date <span style={s.required}>*</span>
			</label>
			<input
				style={{
					...s.select,
					borderColor: errors.startDate ? "#EF4444" : "#E8ECF2",
				}}
				type="date"
			...
			/>
			{errors.startDate && (
				<div style={s.errorText}>{errors.startDate}</div>
			)}
		</div>
		...
		<div style={s.formGroup}>
			...
			<input
				style={{
					...s.select,
					borderColor: errors.endDate ? "#EF4444" : "#E8ECF2",
				}}
				type="date"
				value={form.endDate}
				onChange={(e) =>
