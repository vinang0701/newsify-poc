...
const CommunitiesMgmtPage = () => {
    ...
    async function fetchCommunities(): Promise<Community[]> {
        try {
            const response = await api.get(`${user?.inst_id}/admin/communities`);
                ...
                <section className="flex flex-col py-3 px-4 gap-6">
                    ...
                    <div className="flex flex-row justify-between items-center">
                        ...
                        <ButtonGroup className="flex flex-row">
                            <Input
                                ...
                                className="placeholder:text-border pl-2 border-border rounded-sm w-72"
                            />
                            {searchQuery && (
                                <Button
                    ...
                    {filteredCommunities && filteredCommunities.length > 0 ? (
                        ...
                        <div className="flex justify-center items-center py-20 text-muted-foreground">
                            {searchQuery ? `No communities found for "${searchQuery}"` : "No communities created yet."}
                        </div>
                    )}
                </section>
            </div>
