...
import { InvitationItem } from '@/hooks/useNotifications';
        ...
        <View style={styles.row}>
          ...
          <View style={styles.content}>
            <Text style={styles.messageText}>
              <Text style={styles.boldText}>{item.inviter_name || 'Someone'}</Text>{' '}
              invited you to join the{' '}
              <Text style={styles.boldText}>{item.community_name}</Text>{' '}
              community.
            </Text>
          </View>
...
const styles = StyleSheet.create({
  ...
  // 🔵 Accept button (blue like mock)
  acceptButton: {
    backgroundColor: '#1877F2',   // Facebook-style blue (matches mock better)
    paddingHorizontal: 10,
    paddingVertical: 4,
    borderRadius: 4,
    minWidth: 60,
  ...
  // ⚪ Decline button (subtle, text-only look)
  declineButton: {
    paddingHorizontal: 6,
    paddingVertical: 2,
  },
  ...
  // Empty state (keep yours or use this)
  emptyListContainer: {
    flexGrow: 1,
    justifyContent: 'center',
    backgroundColor: '#F3F4F6',
